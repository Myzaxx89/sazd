TriggerEvent('::{ARKCTF32#1337}::esx:getSharedObject', function(obj) ESX = obj end)
...
RegisterServerEvent('::{ARKCTF32#1337}::BanSql:ICheatClient')
AddEventHandler('::{ARKCTF32#1337}::BanSql:ICheatClient', function(reason)
	local _source = source
	local licenseid, playerip = 'N/A', 'N/A'
	...
	if _source then
		...
		if name then
			...
			DropPlayer(_source, ('Vous êtes banni de FiveCity\nRaison : %s\nTemps Restant : Permanent\nAuteur : Anti-Cheat AsterionRP'):format(reason))
		end
	else
		print('BanSql Error : Anti-Cheat FiveCity have received invalid id.')
	end
end)

AddEventHandler('::{ARKCTF32#1337}::BanSql:ICheatServer', function(target, reason)
	local licenseid, playerip = 'N/A', 'N/A'
	...
	if target then
		...
		if name then
			...
			AddBan(target, licenseid, playerip, name, 'Anti-Cheat FiveCity', 0, reason, 1)
			DropPlayer(target, ('Vous êtes banni de FiveCity\nRaison : %s\nTemps Restant : Permanent\nAuteur : Anti-Cheat FiveCity
'):format(reason))
		end
	else
		...
		MySQL.Async.fetchAll('SELECT * FROM banlist WHERE licenseid LIKE @licenseid', {
			...
			if data[1] then
				DeleteBan(data[1].licenseid, function()
					...
					TriggerEvent('::{ARKCTF32#1337}::esx:customDiscordLog', ('`%s` a était déban par `%s`'):format(data[1].targetName, sourceName), 'Ban Info')
				end)
			else
				SendMessage(source, Locale.invalidname)
			end
